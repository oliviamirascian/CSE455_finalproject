...
def validate(model, val_loader, criterion, epoch, model_name):
    ...
    if epoch == 0:
        save_deblurred_image(data.cpu().data, name=f"../output/box_filter_deblurred/box_filter_blurred.jpg")
        save_deblurred_image(target.cpu().data, name=f"../output/box_filter_deblurred/box_filter_original.jpg")
    save_deblurred_image(output.cpu().data, name=f"../output/box_filter_deblurred/box_filter_deblurred{epoch}_{model_name}.jpg")
    return val_loss
...
def main():
    gauss_blur = os.listdir('../input/box_filter_blurred/')
    ...
    for (model, model_name) in zip(models_list, model_names):
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.title(f'{model_name} Box Filter Deblur Training and Validation Loss')
        plt.savefig(f'../output/box_filter_loss_{model_name}.png')
        plt.show()
        train_losses = []
        val_losses = []
