...
def validate(model, val_loader, criterion, epoch, model_name):
    ...
    if epoch == 0:
        save_deblurred_image(data.cpu().data, name=f"../output/motion_deblurred/motion2_blurred.jpg")
        save_deblurred_image(target.cpu().data, name=f"../output/motion_deblurred/motion2_original.jpg")
    save_deblurred_image(output.cpu().data, name=f"../output/motion_deblurred/motion2_deblurred{epoch}_{model_name}.jpg")
    return val_loss
...
def main():
    motion_blur = os.listdir('../input/motion_blurred/')
    grayscale = os.listdir('../input/grayscaled/')

    x_input = []
    for i in range(len(motion_blur)):
        x_input.append(motion_blur[i])
    ...
    for (model, model_name, label) in zip(models_list, model_names, labels):
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.title(f'{label} Motion Deblur Training and Validation Loss')
        plt.savefig(f'../output/motion2_loss_{model_name}.png')
        plt.show()
        train_losses = []
        val_losses = []
